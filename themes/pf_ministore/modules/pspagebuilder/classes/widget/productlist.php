...
class PsWidgetProductlist extends PsWidgetPageBuilder  {
	...
	public function renderForm($data)
	{
		...
		if ($is_menu = 1 && $this->check_profile == 'Menu') {
			$lists = array(
	...
	public function renderContent($setting)
	{
		...
		switch ($setting['list_type'])
		{
			...
			case 'featured':
				...
				$random   = 1;
				$products = self::getFeaturedProducts($setting['categoryBox'], 1, ($nb ? $nb : 8), $random, $this->context);
				break;
	...
	public static function getProducts($categories, $p = 1, $n, $active = true, Context $context = null, $type = '')
	{
		...
		$validate_approve = Configuration::get('PRODUCT_COMMENTS_MODERATE');
		...
		$sql = 'SELECT p.*, product_shop.*, stock.out_of_stock, IFNULL(stock.quantity, 0) as quantity, 
			...
			Shop::addSqlAssociation('image', 'i', false, 'image_shop.cover=1').'
				...
				LEFT JOIN `'._DB_PREFIX_.'manufacturer` m
						...
						.($type == 'toprating' ? 'JOIN `'._DB_PREFIX_.'product_comment` pc ON (cp.id_product = pc.id_product '.($validate_approve ? ' AND pc.validate = 1' : '').')' : '').'
