...
class PsWidgetCategoriestabs extends PsWidgetPageBuilder {
	...
	public function renderForm($data)
	{
		...
		$modes = array(
			...
			array('value' => 'layout1', 'text' => $this->l('Layout Custom'))
		);
		$this->fields_form[1]['form'] = array(
			'legend' => array(
	...
	public function renderContent($setting)
	{
		...
		if ($categories) {
			...
			foreach ($categories as $id_category) {
				...
				$products = $obj->getProducts($context->language->id, 0, $setting['limit'], $porder[0], $porder[1]);

				$context = Context::getContext();

				$tg['products'] = self::getResults($products, $context);
				$output[] = $tg;
			}
		}
