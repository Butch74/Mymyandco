...
{foreach from=$rows item=row name=row}
    ...
    {if $row->level==1} 
        <div class="pts-container-wrap {$row->cls|escape:'html':'UTF-8'} {if isset($row->parallax)&&$row->parallax} pts-parallax{/if}" {$row->attrs}{* HTML, cannot escape *}>        
            <div class="pts-inner ps-container{if $row->fullwidth==1}-fluid{/if}">
    {/if}    
